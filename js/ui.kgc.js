			...
			toggleAccordian: function toggleAccordian(_scope, eventTarget, evt) {
				...
				target.on(evt, function () {
					...
					if (parent.hasClass(klassCtrl)) {
						...
						if ($('body').find('.history-list').length > 0) {
							parent.parent().siblings().find('.content').slideUp(300, function () {
		...
		if (categoryList.length > 0) {
			categoryList.find('.depth1 > li > h2').on('click', function () {
				if ($(this).hasClass('active')) {
					...
					$(this).parent().siblings().find('> h2').removeClass('active').siblings('.depth2').stop().slideUp(300, function () {});
				}
			});
